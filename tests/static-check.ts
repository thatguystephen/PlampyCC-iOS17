...
assert(workflow.includes("dpkg-deb -R \"$source_package\"") && workflow.includes("source_preferences=") && workflow.includes("lipo \"$source_binary\" -thin \"$arch\""), "workflow does not collect companions from the exact package binary");
