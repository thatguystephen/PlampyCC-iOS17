...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="plampycc-diagnostic-output-") as directory:
        ...
        output = sandbox / "var/jb" / OUTPUT_SUFFIX
        ...
        assert_true(compile_result.returncode == 0,
                    f"shared production-walk regression compile failed:\n{compile_result.stderr}")
        trusted_prefix = sandbox / "var/jb/var/mobile/Library"
        walk_result = subprocess.run(
            [str(binary), str(trusted_prefix), str(sandbox)],
            cwd=ROOT, text=True, capture_output=True,
        )
        assert_true(walk_result.returncode == 0,
                    f"shared production-walk regression failed:\n{walk_result.stdout}{walk_result.stderr}")

        effective_uid = os.geteuid()
        assert_true(output == mobile_parent / "CAML-Diagnostic",
    ...
    print("PASS: rootless symlink regression rejects the legacy walk and the shared production walk creates/validates CAML-Diagnostic/events.jsonl")
