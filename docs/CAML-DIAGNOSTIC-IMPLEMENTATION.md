...
This is the runtime-data path produced by `ROOT_PATH_NS("/var/mobile/Library/Application Support/PlampyCC/CAML-Diagnostic")`, so the macro contributes exactly one `/var/jb` prefix. It is intentionally separate from the packaged, system-wide assets at `/var/jb/Library/Application Support/PlampyCC`; the collector never writes output below that root-owned asset tree. The directory is created by the SpringBoard/mobile writer with mode `0700`, and the file is restricted to `0600`. Directory admission uses a two-zone contract. The trusted prefix `ROOT_PATH_NS("/var/mobile/Library/Application Support")` is the last platform-owned parent and is validated by `ValidatePlatformPrefixDescriptor`: a directory owned by root or the effective user, never world-writable, where the mobile-owned form may be group-writable because the sanctioned mobile data parents ship as `mobile:mobile 0755` or `0775`. The owned suffix `PlampyCC/CAML-Diagnostic` stays strictly descriptor-confined: `O_NOFOLLOW` component opens relative to the held descriptor, `0700` creation, effective-user ownership, no group/world-writable intermediates, and an exact `0700` leaf. This boundary fix admits the observed `0775` platform parent that the earlier strict non-leaf rule rejected (which aborted every flush before the leaf existed) without weakening validation anywhere in the tweak-owned zone. Events use fixed compact JSON keys (`v` version, `t` monotonic milliseconds, `w` wall-clock seconds, `s` site, `p` package name, `x` coarse path prefix, `n` description-is-new, `g` glyph state, `d` description class, `i` view tag, `a` ancestor class, `c` consumer class, `h` construction path, `f` source-URL form, `y` proposed-replacement-URL form, `o` load/rejection outcome, `r` repeat, `q` per-site installation success, `b` build ID, `u` Mach-O UUID).
